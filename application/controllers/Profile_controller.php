...
class Profile_controller extends Home_Core_Controller
{
    ...
    public function change_password_post()
    {
        ...
        if ($old_password_exists == 1) {
            $this->form_validation->set_rules('old_password', trans("form_old_password"), 'required');
        }
        $this->form_validation->set_rules('password', trans("form_password"), 'required|min_length[4]|max_length[50]');
        $this->form_validation->set_rules('password_confirm', trans("form_confirm_password"), 'required|matches[password]');
        if ($this->form_validation->run() == false) {
            $this->session->set_flashdata('errors', validation_errors());
